...
class IndexHandlerCommand(MixinHandler, tornado.web.RequestHandler):

    executor = ThreadPoolExecutor(max_workers=cpu_count()*5)

    def initialize(self, loop, policy, host_keys_settings):
        super(IndexHandlerCommand, self).initialize(loop)
        self.policy = policy
        self.host_keys_settings = host_keys_settings
        self.ssh_client = self.get_ssh_client()
        self.debug = self.settings.get('debug', False)
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(IndexHandlerCommand, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(IndexHandlerCommand, self).check_origin(origin):
                raise tornado.web.HTTPError(
                    403, 'Cross origin operation is not allowed.'
                )
    ...
    def get(self):
        self.render('command.html', debug=self.debug, font=self.font)
